/**
 * Leaderboard API: Vibers This Week (default) and Claimed Vibes.
 * GET /api/leaderboard?view=week|claimed
 */
...
export async function GET(req: NextRequest) {
  ...
  if (view !== "week" && view !== "claimed") {
    return NextResponse.json(
      { error: "Invalid view; use week or claimed" },
      { status: 400 }
    );
  }
  ...
  type Agg = { count: number; latestClaimedAt: string; maskedWallet: string };
  const bySender = new Map<string, Agg>();
